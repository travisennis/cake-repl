...
1. Run `ahm prime` before starting work and re-run it after context compaction.
   It reports workflow state, validation warnings, and the scoped intake command
   for the work at hand.
2. Do scoped managed-work intake when applicable.
   ...
   - If the request is directly about code, CLI behavior, tests, docs, build,
     release, or repo mechanics, skip scoped intake and classify directly after
     `ahm prime`.
3. Classify the request — choose the workflow route below.
4. Load only the routed docs needed for that request.
5. Apply the Docs And ADR Gate before completion or handoff.
6. State the selected route and loaded docs in handoff.
7. Preserve compatibility surfaces unless explicitly changed.
8. Keep edits surgical and verify according to risk (see the testing route).
9. Hand off with changes, exact checks, and remaining risk.
...
Workflow records live under `.ahm/`: tasks in `.ahm/tasks/`, ExecPlans in
`.ahm/exec-plans/`, and research notes in `.ahm/research/`. ADRs are durable
project documentation under `docs/adr/`, with lifecycle managed through `ahm`.
...
- Research: run `ahm context research` and use `.ahm/research/index.md` as
...
Never hand-edit indexes generated by `ahm`; update source records and run the
appropriate `ahm` command. Use `ahm task` for task state changes and `ahm adr`
for ADR lifecycle changes.
