...
- **Preserve the cake engine contract.** Live prompts run only as
  ...
  `--profile`/`--tools`/`--add-dir` and `--` before the prompt. Startup
  `-resume` may additionally run the read-only `cake --output-format
  stream-json replay <uuid>` command. Never read cake session files, parse its
  human text, or import cake internals. The REPL drives cake purely through the
  NDJSON stream.
